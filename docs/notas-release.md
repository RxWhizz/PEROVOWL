...
<https://github.com/RxWhizz/PEROVOWL/releases/tag/v0.7.1>

En **Assets**, descarga el paquete que corresponda a tu sistema:

| Sistema | Archivo recomendado | Uso |
|---|---|---|
| Windows 10/11 x64 | `dft-monitor-desktop-0.7.1-windows-x64.zip` | GUI de escritorio nativa con motor local embebido |
| Debian/Ubuntu x64 | `perovowl-dft-monitor-0.7.1-linux-amd64.deb` | GUI de escritorio instalable en el sistema |
| Linux x86_64 portable | `dft-monitor-desktop-0.7.1-linux-x86_64.tar.gz` | GUI portable sin instalador |
| Linux servidor/web | `dft-monitor-web-0.7.1-linux-x86_64.tar.gz` | Servidor local que abre la interfaz en navegador |
...
Descarga `dft-monitor-desktop-0.7.1-windows-x64.zip`, descomprimelo **en una
carpeta corta** (p. ej. `C:\perovowl`) y ejecuta el `.exe` desde dentro de la
carpeta extraida:

```powershell
Expand-Archive .\dft-monitor-desktop-0.7.1-windows-x64.zip -DestinationPath C:\perovowl
C:\perovowl\dft-monitor-desktop-0.7.1-windows-x64\dft_monitor_flutter.exe
```
...
Descarga `perovowl-dft-monitor-0.7.1-linux-amd64.deb` e instalalo con:

```bash
sudo apt install ./perovowl-dft-monitor-0.7.1-linux-amd64.deb
...
```bash
tar xzf dft-monitor-desktop-0.7.1-linux-x86_64.tar.gz
./dft-monitor-desktop-0.7.1-linux-x86_64/dft_monitor_flutter
```
...
```bash
tar xzf dft-monitor-web-0.7.1-linux-x86_64.tar.gz
...
```powershell
Get-FileHash .\dft-monitor-desktop-0.7.1-windows-x64.zip -Algorithm SHA256
Get-FileHash .\perovowl-dft-monitor-0.7.1-linux-amd64.deb -Algorithm SHA256
```
